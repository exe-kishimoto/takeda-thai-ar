...
## 🎚 揺れ（ジッター）対策
MindARの手ブレ補正（One Euroフィルタ）を強めに設定しています
（`index.html` の `mindar-image="... filterMinCF: 0.00005; filterBeta: 5 ..."`）。
まだ気になる場合は `filterBeta` を `3` 、`filterMinCF` を `0.00003` など下げるとより滑らかになります
（下げるほど揺れは減りますが、動きへの追従は少し遅くなります）。明るい場所・写真を平らに保つのも有効です。
